...
const JWT_SECRET = process.env.JWT_SECRET || 'denr-car-inventory-secure-secret-2026';
...
app.use((req, res, next) => {
    // Skip token check for login, root, or non-API routes (like fetching React files)
    if (req.path === '/api/login' || req.path === '/' || !req.path.startsWith('/api/')) {
        return next();
    }
    ...
    jwt.verify(token, JWT_SECRET, (err, decodedUser) => {
        ...
        // Attach the real user data (from the token) to req.user
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        // We pack the user's ID and Division directly into the secure token
        const tokenPayload = {
            id: user.id,
            email: user.email,
        ...
        const token = jwt.sign(tokenPayload, JWT_SECRET, { expiresIn: '12h' }); // Token expires in 12 hours

        // Send success response back to React
        res.status(200).json({
            user: user,
            token: token // This is now a real, encrypted JWT
        });
...
const pool = mysql.createPool({
    ...
    port: process.env.MYSQLPORT || 53638,
    ...
    try {
        ...
        for (const item of items) {
            ...
            if (!stockInfo.stockCardId || quantityRequested > stockInfo.remaining) {
                ...
                stockInfo.remaining -= quantityIssued;
            }

            // FIX: Now capturing unitPrice and amount!
        ...
        const childValues = normalizedItems.map((i) => [id, i.stockNo, i.description, i.unit, i.quantityRequested, i.quantityIssued, i.remarks, i.unitPrice, i.amount]);
        await connection.query(
            `INSERT INTO ris_items (ris_record_id, stock_no, description, unit, quantity_requested, quantity_issued, remarks, unit_price, amount) VALUES ?`, 
            [childValues]
        );
        ...
        for (const item of normalizedItems) {
            ...
            const currentBalance = stockDeductionState.has(item.stockNo) ? stockDeductionState.get(item.stockNo) : stockInfo.balance;
...
app.get('/api/rsmiRecords', async (req, res) => {
    try {
        const [records] = await pool.query(`
            SELECT id, report_no AS reportNo, period, total_value AS totalValue, is_auto_generated AS isAutoGenerated
            FROM rsmi_records 
...
app.post('/api/rsmiRecords', async (req, res) => {
    // FIX: Extract totalValue from req.body
    const { reportNo, period, items, totalValue } = req.body; 
    
    if (!reportNo || reportNo.trim() === '') {
        return res.status(400).json({ error: 'Report Number is required and cannot be empty.' });
    }
    ...
    try {
        ...
        // FIX: Added total_value to the INSERT query
        await connection.query(
            `INSERT INTO rsmi_records (id, report_no, period, is_auto_generated, source_ris_number, total_value) VALUES (?, ?, ?, ?, ?, ?)`, 
            [id, reportNo, period, isAutoGenerated, sourceRisNumber, totalValue || 0]
        );
...
app.put('/api/rsmiRecords/:id', async (req, res) => {
    ...
    try {
        ...
        if (rsmiRecords.length === 0) return res.status(404).json({ error: 'RSMI record not found' });
        
        if (rsmiRecords[0].is_auto_generated) {
            return res.status(403).json({ error: 'Auto-generated RSMI records cannot be edited.' });
        }
        
        await connection.beginTransaction();
        
        // FIX: Added total_value to the UPDATE query
        await connection.query(
            `UPDATE rsmi_records SET report_no = ?, period = ?, total_value = ? WHERE id = ?`, 
            [reportNo, period, totalValue || 0, id]
        );
